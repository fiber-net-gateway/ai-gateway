...
| 模块           | 模型调用权限申请、管理员审批、Provider 授权组、rnacos 发布            |
| 文档状态       | 需求基线                                                              |
| 编写日期       | 2026-08-03                                                            |
...
- 每个需要审批的模型绑定一个“申请授权组”。该组由模型的一个主 Provider 托管，便于明确
  业务负责人和复用范围，但运行时仍是模型级授权。
- 用户进入该组后，可以调用引用该组的全部逻辑模型；请求仍可能命中这些模型配置的任意主
  Provider 或 Fallback。页面必须在审批影响范围中明确列出这些模型。
...
- 管理员查看待审批队列、申请人、模型、Provider 托管组和完整影响模型范围。
- 管理员批准或拒绝；申请人不能审批自己的申请。
- 批准时把申请人的精确 username 加入 Provider 托管的申请授权组，并创建不可变发布记录。
...
从 `ALL_AUTHENTICATED` 切换到 `APPROVAL_REQUIRED` 时，后端为排序最靠前的主 Provider
创建或复用一个托管组，并把该组写入模型草稿。模型创建后，托管关系保持稳定，不因 Provider
展示名或排序变化自动迁移。若托管 Provider 被解绑，应阻止保存并要求管理员先调整访问策略。
...
一个 Provider 在一个环境内最多有一个申请授权组。共享 Provider 可使多个模型引用同一组。
批准一个申请时，管理员必须看到“加入此组后还将获得哪些模型”的列表。该列表来自当前
已发布模型快照，不能只根据申请页面缓存推断。

组名由服务端根据 Provider 不可变标识生成，格式为 `pa_<prefix>_<digest>`；客户端不能指定、
修改或复用名称。组名在创建后永久稳定。
...
- `不可申请`：模型配置不完整、没有托管申请组、模型已归档或环境访问已撤销。
...
已发布成员、模型没有托管申请组、已有待审批申请或已有批准申请。
...
| `model_access.request.approved`            | requestId、groupId、providerId、affectedModelIds、publicationId |
...
- Provider group 内容按 username byte order 排序，重复批准不产生重复成员或非确定性 JSON。
- 所有按钮支持键盘操作；状态使用文字和图标，不只依赖颜色。
- 移动端审批卡片按申请、审批、发布、生效顺序纵向展示。

## 11. 验收标准

1. 管理员可把模型设置为“需要审批”；草稿显示托管 Provider 和组，发布前不宣称已生效。
2. 普通用户对受限已发布模型提交用途说明，重复待审批申请被阻止。
3. 普通用户不能读取其他用户的申请或组成员。
4. 管理员看到 Provider 组复用导致的全部模型授权影响。
