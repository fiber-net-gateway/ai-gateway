...
AUDIT_HTTP_BATCH_SIZE=50
AUDIT_HTTP_BATCH_MAX_BYTES=524288
AUDIT_HTTP_FLUSH_INTERVAL_MS=500
AUDIT_HTTP_CONNECT_TIMEOUT_MS=1500
AUDIT_HTTP_REQUEST_TIMEOUT_MS=7500
)";
    ...
    EXPECT_EQ(audit.queue_capacity_bytes, 1048576u);
    EXPECT_EQ(audit.batch_size, 50u);
    EXPECT_EQ(audit.batch_max_bytes, 524288u);
...
TEST(AiServerConfigTest, RejectsInvalidAuditDeliverySettings) {
    auto short_token = AiServerConfig::load_from_string("NACOS_SERVER_ADDRESSES=127.0.0.1\n"
                                                        "AI_SERVER_LOG_CONFIG_PATH=ai-server.logging.json\n"
                                                        "AUDIT_INGEST_TOKEN=too-short\n");
    ASSERT_FALSE(short_token);
    EXPECT_EQ(short_token.error().code, AiServerConfigErrorCode::InvalidValue);
    EXPECT_EQ(short_token.error().key, "AUDIT_INGEST_TOKEN");

    auto oversized_batch = AiServerConfig::load_from_string("NACOS_SERVER_ADDRESSES=127.0.0.1\n"
                                                            "AI_SERVER_LOG_CONFIG_PATH=ai-server.logging.json\n"
                                                            "AUDIT_HTTP_BATCH_SIZE=101\n");
    ASSERT_FALSE(oversized_batch);
