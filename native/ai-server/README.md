...
顶层 CMake 参数 `AI_SERVER_AUDIT_TRANSPORT` 决定唯一的审计传输实现，取值为 `FILE`
或 `HTTP`，默认 `HTTP`。它是编译期选择，不允许运行时同时写文件和调用接口：

- `FILE` 保留上游 schema v5 NDJSON 审计文件、轮转、恢复和日志指标；
- `HTTP` 不创建审计 FileAppender。ai-server 固定订阅 Nacos 服务
  `AI-GATEWAY@@ai-server-console-api`，只选择 enabled、healthy 且权重大于零的 IP/port，
  通过后台线程向 `/api/internal/llm-call-audits/batches` 提交有界批次；请求 EventLoop
  不等待控制台接口；
- HTTP 队列满、记录超限或控制台暂时不可用不会改变 LLM 请求结果。可重试错误采用退避，
  进程内队列不会持久化，停机仍无法送达的记录会计入丢弃指标。

HTTP 模式要求 `AUDIT_INGEST_TOKEN` 与 console-api 配置一致，且至少包含 32 个无空白
字节。`AI_SERVER_INSTANCE_ID` 标识来源实例；队列、批量、刷新和连接/请求超时可通过
`AUDIT_HTTP_*` 调整，完整示例见 [`ai-server.env.example`](ai-server.env.example)。为兼容
同一份日志 JSON，HTTP 模式仍校验 `audit.max_record_bytes`，但忽略审计文件路径和轮转
设置。
...
服务实例固定使用基于 route key 和 Nacos 权重的 Rendezvous Hash。实例级失败后的同
Provider 重试会排除本次请求已经失败的实例。
