...
AiServerMetrics::collect(mem::IoBufNodePool &node_pool, TokenRateLimiterStats limiter_stats, std::size_t cluster_nodes,
                         const log::AppenderStats *audit_stats,
                         const LlmAuditDeliveryStats *audit_delivery_stats) noexcept {
    ...
    if (audit_delivery_stats &&
        (!append_counter(output, "ai_server_audit_http_queued_records_total", "LLM audit records queued for HTTP.",
                         audit_delivery_stats->queued_records) ||
         !append_counter(output, "ai_server_audit_http_delivered_records_total",
                         "LLM audit records accepted over HTTP.", audit_delivery_stats->delivered_records) ||
         !append_counter(output, "ai_server_audit_http_dropped_records_total",
                         "LLM audit records dropped by HTTP delivery.", audit_delivery_stats->dropped_records) ||
         !append_counter(output, "ai_server_audit_http_retryable_failures_total",
                         "Retryable LLM audit HTTP batch failures.", audit_delivery_stats->retryable_failures) ||
         !append_counter(output, "ai_server_audit_http_permanent_failures_total",
                         "Permanently rejected LLM audit HTTP batches.", audit_delivery_stats->permanent_failures) ||
         !append_gauge(output, "ai_server_audit_http_queue_bytes", "Queued LLM audit HTTP payload bytes.",
                       audit_delivery_stats->queue_bytes) ||
         !append_gauge(output, "ai_server_audit_http_endpoints", "Healthy console API audit endpoints.",
                       audit_delivery_stats->endpoint_count))) {
        co_return std::unexpected(output.error());
    }
