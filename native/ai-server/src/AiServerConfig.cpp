...
constexpr std::string_view kAuditInstanceIdKey = "AI_SERVER_INSTANCE_ID";
constexpr std::string_view kAuditIngestTokenKey = "AUDIT_INGEST_TOKEN";
constexpr std::string_view kAuditQueueBytesKey = "AUDIT_HTTP_QUEUE_CAPACITY_BYTES";
constexpr std::string_view kAuditBatchSizeKey = "AUDIT_HTTP_BATCH_SIZE";
constexpr std::string_view kAuditBatchBytesKey = "AUDIT_HTTP_BATCH_MAX_BYTES";
...
bool parse_size(std::string_view text, std::size_t minimum, std::size_t maximum, std::size_t &out) noexcept {
    std::uint64_t value = 0;
    const auto result = std::from_chars(text.data(), text.data() + text.size(), value);
    if (text.empty() || result.ec != std::errc{} || result.ptr != text.data() + text.size() || value < minimum ||
        value > maximum) {
        return false;
    }
...
apply_entry(const EnvEntry &entry, net::IpAddress &listen_ip, std::uint16_t &listen_port,
    ...
    if (entry.key == kAuditInstanceIdKey) {
        ...
        audit_options.instance_id = entry.value;
        return {};
    }
    if (entry.key == kAuditIngestTokenKey) {
        ...
        audit_options.ingest_token = entry.value;
        return {};
    }
    ...
    if (entry.key == kAuditBatchSizeKey) {
        if (!parse_size(entry.value, 1, 100, audit_options.batch_size)) {
            return std::unexpected(make_error(AiServerConfigErrorCode::InvalidValue, entry.line, entry.key,
                                              "expected 1 through 100 records"));
        }
        return {};
    }
    if (entry.key == kAuditBatchBytesKey) {
        if (!parse_size(entry.value, 64 * 1024, 8 * 1024 * 1024, audit_options.batch_max_bytes)) {
            return std::unexpected(make_error(AiServerConfigErrorCode::InvalidValue, entry.line, entry.key,
                                              "expected 65536 through 8388608 bytes"));
        }
        return {};
    }
    if (entry.key == kAuditFlushIntervalKey || entry.key == kAuditConnectTimeoutKey ||
        entry.key == kAuditRequestTimeoutKey) {
        std::chrono::milliseconds parsed;
        if (!parse_milliseconds(entry.value, parsed) || parsed < std::chrono::milliseconds(1) ||
            parsed > std::chrono::minutes(5)) {
            return std::unexpected(make_error(AiServerConfigErrorCode::InvalidValue, entry.line, entry.key,
                                              "expected 1 through 300000 milliseconds"));
        }
        if (entry.key == kAuditFlushIntervalKey) {
            audit_options.flush_interval = parsed;
        } else if (entry.key == kAuditConnectTimeoutKey) {
            audit_options.connect_timeout = parsed;
        } else {
            audit_options.request_timeout = parsed;
        }
        return {};
    }
#endif
